...
router.get('/', async (req, res) => {
  const items = await Solidarity.find().sort({ createdAt: -1 });
  res.json(items);
});
...
router.post('/:id/like', auth, async (req, res) => {
  const item = await Solidarity.findById(req.params.id);
...
router.delete('/:id', auth, async (req, res) => {
  ...
  logIfNotOwnerContentDelete(req, item, 'solidarity');
