...
@Component({
  ...
  template: `
    ...
    <app-modal [(visible)]="modalVisible" title="Nouvelle publication solidarité">
      <form [formGroup]="itemForm" (ngSubmit)="submit()" class="form-modal">
        <div class="form-group">
          <label class="form-label">Titre *</label>
          <input type="text" formControlName="title" placeholder="Ex: Besoin d'aide pour un projet associatif" class="form-control">
        ...
        <div class="form-group">
          ...
          <input type="text" formControlName="type" placeholder="Ex: aide, demande, don, bénévolat..." class="form-control">
          <div class="text-muted" style="font-size:0.875rem; margin-top:4px;">Optionnel</div>
        </div>

        <div class="form-group">
          <label class="form-label">Description *</label>
          <textarea formControlName="desc" placeholder="Expliquez votre besoin / proposition de solidarité..." rows="5" class="form-control"></textarea>
...
export class SolidariteComponent implements OnInit {
  ...
  loadItems() { this.api.get('solidarity').subscribe((data: any) => { this.items = data; this.updateFilter(); }); }
  ...
  async submit() {
    ...
    try {
      ...
      await this.api.post('solidarity', formValue).toPromise();
  ...
  toggleLike(item: any) { this.api.post('solidarity/' + item.id + '/like', {}).subscribe(() => this.loadItems()); }
