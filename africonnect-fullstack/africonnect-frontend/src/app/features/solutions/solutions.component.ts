...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ModalComponent, TranslateModule, PublishLocationStepComponent],
  template: `
    ...
    <app-modal [(visible)]="modalVisible" [title]="(editingItem ? 'sections.solutionsEdit' : 'sections.solutionsNew') | translate">
      ...
      <form *ngIf="editingItem || !locationStepActive" [formGroup]="itemForm" (ngSubmit)="submit()" class="form-modal">
        <div class="form-group">
          ...
          <input type="text" formControlName="title" placeholder="Ex: Tutoriel Angular 17 - Bonnes pratiques" class="form-control">
        ...
        <div class="form-group">
          ...
          <input type="text" formControlName="category" placeholder="Ex: Développement, Business, Santé..." class="form-control">
          <div class="text-muted" style="font-size:0.875rem; margin-top:4px;">Optionnel</div>
        </div>

        <div class="form-group">
          <label class="form-label">Description *</label>
          <textarea formControlName="desc" placeholder="Décrivez la solution, étapes, liens utiles..." rows="5" class="form-control"></textarea>
...
export class SolutionsComponent implements OnInit {
  ...
  deleteItem(id: string) {
    if (confirm(this.translate.instant('common.confirmDelete'))) this.api.delete('solutions/' + id).subscribe(() => this.loadItems());
  }
  canDelete(item: any) {
  ...
  toggleLike(item: any) {
    ...
    this.api.post('solutions/' + item._id + '/like', {}).subscribe({
